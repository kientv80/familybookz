...
public class DumyDB {
	...
	private Person toPerson(ResultSet rs, Connection con, boolean loadChildren) throws SQLException {
		...
		p.setWifeorhusbandIds(rs.getString("wifeorhusband_ids"));
		p.setMom(rs.getLong("mom"));
		p.setDad(rs.getLong("dad"));
		...
		if (loadChildren) {
			if (p.getGender().equals(GENDER.MALE)) {
				p.setChildren(getChildren(con, p.getId(), true));
			} else {
				p.setChildren(getChildren(con, p.getId(), false));
			}
		}
	...
	public Family login(String username, String password) throws Exception {
		...
		ResultSet rs = null;
		Person me = null;
		List<Person> enagedRelations = null;
		Family f = null;
		try {
			stm = con.prepareStatement(sql);
			...
			if (me != null && me.getWifeorhusbandIds() != null && !me.getWifeorhusbandIds().isEmpty()) {
				enagedRelations = new ArrayList<>();
				getWifeOrHusband(con, enagedRelations, me.getWifeorhusbandIds());
			}
			if (me != null) {
	...
	public long postFeed(Feed f, Person p) throws Exception {
		String sql = "insert into feed(type,content,images,title,url,owner_id,owner_name,owner_avatar,description,website) values (?,?,?,?,?,?,?,?,?,?)";
		Connection con = JDBCConnection.getInstance().getConnection();
		PreparedStatement stm = null;
		ResultSet rs = null;
		long id = 0;
	...
	public List<Feed> getFeeds(long ownerId) throws Exception {
		String sql = "SELECT feed_ids FROM profile_feeds_timeline where profile_id = ?";
		Connection con = JDBCConnection.getInstance().getConnection();
		PreparedStatement stm = null;
		ResultSet rs = null;
		...
		try {
			...
			String ids = "";
			while (rs.next()) {
				ids = rs.getString("feed_ids");
			}
			if (!ids.isEmpty()) {
				rs.close();
				rs = stm.executeQuery("select * from feed where id in (" + ids + ") order by id desc");
				while (rs.next()) {
					Feed f = toFeed(rs);
					feeds.put(f.getId(), f);
				}
	...
	private Feed toFeed(ResultSet rs) throws SQLException {
		...
		ShortProfile s = new ShortProfile(rs.getLong("owner_id"), rs.getString("owner_name"), rs.getString("owner_avatar"));
		f.setOwnerInfo(s);
		return f;
	}
	...
	enum RELATION_TYPE {
		MOM, DAD, PROTHER_OR_SYSTER, WIFE_OR_HUSBAND
	}
	...
	public void confirmRequest(long id) throws Exception {
		...
		try {
			...
			if ("mom".equals(type)) {
				...
				if (requesting.getWifeorhusbandIds() == null || requesting.getWifeorhusbandIds().isEmpty()) {
					requesting.setWifeorhusbandIds(p.getId() + "");
				} else {
					requesting.setWifeorhusbandIds(requesting.getWifeorhusbandIds() + "," + p.getId());
				}
			...
			if ("kid".equals(req.getRelation())) {
				if (GENDER.MALE.equals(requesting.getGender())) {
					p.setDad(req.getRequestingId());
					stm.setLong(3, RELATION_TYPE.DAD.ordinal());
				} else {
					p.setMom(req.getRequestingId());
					stm.setLong(3, RELATION_TYPE.MOM.ordinal());
				}
			...
			DumyDB.getInstance().updateRelation(requesting);
	...
	public List<ShortProfile> getShortProfiles(String ids) throws Exception {
		...
		try {
			...
			rs = stm.executeQuery();
			while (rs.next()) {
				r.add(new ShortProfile(rs.getLong("id"), rs.getString("firstname") + " " + rs.getString("surname"), rs.getString("image")));
			}
		} finally {
			if (stm != null)
				stm.close();
	...
	public long postComment(Comment c) throws Exception {
		String sql = "insert into comment(owner_id,reply_comment_id,feed_id,content,profile_avatar,profile_name,title,url,image,description,type) values (?,?,?,?,?,?,?,?,?,?,?)";
		Connection con = JDBCConnection.getInstance().getConnection();
		PreparedStatement stm = null;
		ResultSet rs = null;
		long id = 0;
		...
		try {
			for (Entry<Long, Action> f : feedActionMapping.entrySet()) {
				if (f.getValue().getCommetnIds() != null && !f.getValue().getCommetnIds().isEmpty()) {
					...
					while (rs.next()) {
						Comment c = new Comment(rs.getLong("id"), rs.getLong("reply_comment_id"), rs.getLong("owner_id"), rs.getLong("feed_id"), rs.getString("content"), rs.getString("profile_name"), rs.getString("profile_avatar"));
